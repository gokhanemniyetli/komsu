...
export const QuestionResultScreenData = (data) => {
    return (dispatch) => {
        operation(dispatch);

        var webServerUrl = GLOBAL.WEB_SERVICE_URL;
        var webService = 'QuestionResult';
...
const operation = (dispatch) => {
    dispatch({
        type: QUESTION_RESULT_SCREEN_DATA
    });
};
...
const operationSuccess = (dispatch, res, userToken) => {
    //debugger;
    dispatch({
        type: QUESTION_RESULT_SCREEN_DATA_SUCCESS,
        payload: res,
        user: userToken
    });
};
...
const operationFail = (dispatch, errorMessage) => {
    ...
    dispatch({
        type: QUESTION_RESULT_SCREEN_DATA_FAIL
    });
};
